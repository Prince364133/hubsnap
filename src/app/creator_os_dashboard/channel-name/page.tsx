...
function ChannelNameSelectionContent() {
    ...
    useEffect(() => {
        async function fetchNames() {
            if (idea && lang) {
                try {
                    const names = await generateChannelNames(idea, lang);
                    setGeneratedNames(names);
                } catch (e) {
                    console.error("Failed to generate names", e);
                } finally {
                    setLoading(false);
                }
            }
    ...
    const handleContinue = async () => {
        if (selectedName && user) {
            ...
            const locked = await dbService.lockChannelName(user.uid, selectedName);

            if (locked) {
                router.push(`/creator_os_dashboard/logo-selection?ideaId=${ideaId}&lang=${lang}&name=${encodeURIComponent(selectedName)}`);
            } else {
                setError("Failed to reserve name. Try again.");
                setIsChecking(false);
            }
        } else if (!user) {
            setError("You must be logged in to claim a name.");
        }
    };
