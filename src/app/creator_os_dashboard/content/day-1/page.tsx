    ...
    useEffect(() => {
        async function loadContent() {
            ...
            try {
                ...
                if (user) {
                    await dbService.saveContentHistory(user.uid, {
                        userId: user.uid,
                        title: idea.name,
                        type: 'mixed',
                        content: generated,
                        ...
                        <div className="grid md:grid-cols-2 gap-6">
                            <Section title="Image Prompts" icon={ImageIcon}>
                                <div className="space-y-3">
                                    {content.imagePrompts?.map((prompt: string, i: number) => (
                            ...
                            <Section title="Stock Video Search" icon={Video}>
                                <ul className="list-disc list-inside space-y-2 text-sm text-text-secondary">
                                    {content.sop.shots.slice(0, 3).map((shot: string, i: number) => (
                                        <li key={i}>{shot}</li>
                                    ))}
                                </ul>
                            </Section>
                    ...
                    <div className="lg:col-span-1 space-y-6">
                        <Card className="p-6 border-red-100 bg-red-50/20">
                            ...
                            <ul className="space-y-2 text-sm text-red-800">
                                {content.sop.textOverlays.map((overlay: string, i: number) => (
