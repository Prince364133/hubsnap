...
export default function ChannelDetailPage() {
    ...
    async function loadChannel() {
        if (params.id && user) {
            const data = await dbService.getChannel(user.uid, params.id as string);
            setChannel(data);
            if (data) {
    ...
    const handleConnect = async () => {
        ...
        setTimeout(async () => {
            await dbService.updateChannel(user.uid, channel.id, {
                connected: true,
                ytStats: {
    ...
    const handleDelete = async () => {
        ...
        if (confirm("Are you sure? This is PERMANENT and cannot be undone.")) {
            await dbService.deleteChannel(user.uid, channel.id);
